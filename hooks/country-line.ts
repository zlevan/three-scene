...
export const useCountryLine = () => {
  ...
  const createCountryFlatLine = (
    data,
    materialOptions: ConstructorParameters<THREE.LineBasicMaterial>[0] = {},
  ...
  const createLine = (
    points: ConstructorParameters<THREE.Vector3>[],
    material: ConstructorParameters<THREE.LineBasicMaterial>[0],
    lineType: 'Line' | 'LineLoop' | 'LineSegments' | 'Line2' = 'LineLoop'
  ) => {
    let line: InstanceType<typeof Line2>
    if (lineType === 'Line2') {
