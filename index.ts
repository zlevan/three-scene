...
const { createCruise, cruiseAnimate, updateCruise, bindEvent, removeEvent } =
  useCruise();
export default class ThreeScene {
  ...
  initLight() {
    ...
    if (directionalLight.visible) {
      ...
      if (directionalLight.light2) {
        ...
        if (directionalLight.helper) {
          const dirLigh2tHelper = new THREE.DirectionalLightHelper(
            dirLight2,
            1
          );
  ...
  exportImage() {
    ...
    link.href = this.renderer.domElement
      .toDataURL()
