...
export const useMaterial = () => {
  // 改变透明
  const changeTransparent = (mode: InstanceType<typeof THREE.Mesh>, opacity = 0.5) => {
    // 改变透明度
    const change = (mesh: any) => {
  ...
  const materialReplace = (
    ...
    if (mesh.receiveShadowName.find((it: string) => name.indexOf(it) > -1)) {
      // 接收阴影
      child.receiveShadow = true
      const glisten = opts.groundReflection
      child.material = materialOptimize(child.material, glisten, opts.side)
    } else if (animateMeshName.find((it: string) => name.indexOf(it) > -1)) {
      // 动画材质
  ...
  // 导出 glb、gltf 文件
  const exportGlb = (model: any, animations: any[], name: string, isGlb: boolean = true) => {
    if (!model) return
