...
export const useFlywire = (options: Params = {}) => {
  ...
  // 创建飞线-动态
  const createFly = (points: InstanceType<typeof THREE.Vector3>[]) => {
    const indexList = new Float32Array(points.map((_: any, index: number) => index))
    // 根据点位创建几何体
  ...
  // 创建坐标点
  const createCroodPoint = (crood: number[]) => {
    const [x, z] = crood
